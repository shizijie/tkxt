	...
	$.addPage = function(f, p) {
		...
		if (p.page_table != false) {
			...
			_a
					...
					.append(
							'当前<b class="fontred"><input name="page.pageNo" type="hidden" value="'
									+ p.page_pageNo + '"/>' + p.page_pageNo
									+ '</b>/' + p.page_totalPages + '['
									+ p.page_totalCount
									+ ']页&nbsp;&nbsp;&nbsp;&nbsp;');
			_a.append($('<a href="#">首页</a>').addClass("page_table_footer_a")
					.click(function(e) {
						jumpPage(1)
					}));
			if (p.page_pageNo > 1) {
				_a.append($('<a href="#">上一页</a>').addClass(
						"page_table_footer_a").click(function(e) {
					jumpPage(p.page_pageNo - 1)
				}));
			}
			if (p.page_pageNo < p.page_totalPages) {
				_a.append($('<a href="#">下一页</a>').addClass(
						"page_table_footer_a").click(function(e) {
					jumpPage(p.page_pageNo + 1)
				}));
			}
			...
			_s = $('<select style="width:60px" name="' + p.page_pageSize_Name + '">');
			switch (p.page_pageSize) {
			case 15:
				_s.append('<option value="15" selected>15</option>')
				...
				break;
			case 50:
				_s.append('<option value="15">15</option>')
			...
			case 100:
				_s.append('<option value="15">15</option>')
				  .append('<option value="50">50</option>')
				  .append('<option value="100" selected>100</option>')
				  .append('<option value="200">200</option>')
				  .append('<option value="500">500</option>');
				break;
			case 200:
				_s.append('<option value="15">15</option>')
				  .append('<option value="50">50</option>')
				  .append('<option value="100">100</option>')
				  .append('<option value="200" selected>200</option>')
				  .append('<option value="500">500</option>');
				break;
			case 500:
				_s.append('<option value="15">15</option>')
				  ...
				  .append('<option value="100">100</option>')
				  .append('<option value="200">200</option>')
				  .append('<option value="500" selected>500</option>');
			}
			_a.append(_s);
			_a.append($('<a href="#"> GO </a>').addClass("page_table_footer_a")
					.click(reLoad));
			t.after(_a);
			delete _a;
		}
