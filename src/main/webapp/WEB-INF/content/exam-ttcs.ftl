	...
	<div class="page-container row-fluid">
		...
		<div class="page-content">
			...
			<div class="container-fluid">
					...
					<div class="span10">
								...
								<div class="row-fluid">
									<div class="span12">
										<div class="form-actions">
											<center>
												<button class="btn big" type="button" id="commit" name="commit" onclick="lastWt()" >
													<i class="icon-arrow-left"></i> 上一题
												</button>
												&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
												<button class="btn big" type="button" id="commit" name="commit" onclick="nextWt()" >
													<i class="icon-arrow-right"></i> 下一题
												</button>
												&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
												<button class="btn big green" type="button" onclick="save()" >
													<i class="icon-home"></i> 保存进度
												</button>
												&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
						...
						<div class="span4">
							<div class="row-fluid">
								...
								<div class='alert alert-success' id="dttjs"><center><h4>正确数：<#if ttcsjg?exists>${ttcsjg.ztzql!}<#else>0</#if>&nbsp;&nbsp;错误数：<#if ttcsjg?exists>${ttcsjg.ztcwl!}<#else>0</#if></h4></center></div>
				...
				<!--提醒行-->
				<input id="ttid" type="hidden" value="${ttid!}"></input>
				...
				<script>
					...
					var num=0;
					<#if ttcsjg?exists>
						var zqts = ${ttcsjg.ztzql!};
						var cwts = ${ttcsjg.ztcwl!};
					<#else>
						var cwts = 0;
						var zqts = 0;
					</#if>
					...
					<#list listwt as ob>
						...
						biaogestr += "<td style='background-color:<#if ob.sfyx?exists&&(ob.sfyx=="F")>red";
						<#else><#if ob.sfyx?exists&&(ob.sfyx=="T")>white";<#else>#DDDDDD";</#if></#if>
						biaogestr +=";cursor:pointer' onclick='toshenchenwt("+(${ob.sxh!}-1)+")' id='bg_"+wtArr[${ob.sxh!}-1].id+"' pid='"+(${ob.sxh!}-1)+"'><h4><center>"+${ob.sxh!}+"</center></h4></td>";
						if(${ob.sxh!}%10 == 0 || ${ob.sxh!} == totalCount)
							biaogestr += "</tr><tr>";
							
						//本次测试用题所有id
						utidstr += wtArr[${ob.sxh!}-1].id + ","
					</#list>
					<#else>
					...
					function jiaojun()
					{
						...
						$.ajax({
						   ...
						   async:false,
						   data: "utidstr="+utidstr+"&resultSelectStr="+hdjgArr,
						   success: function(msg){
						    	alert("已交卷");
						    	window.close();
						   },
					...
					function save(){
						var ttid=$("#ttid").val();
						//console.log("套题id:"+ttid+"结果："+ztjg+"正确题数："+zqts+"错误题数："+cwts);
						$.post(
							"${base}/exam!savettcsjg.action",
							{ ttid:ttid,jg:ztjg,zql:zqts,cwl:cwts },
							function(data){
								alert(data);
							}
						);
